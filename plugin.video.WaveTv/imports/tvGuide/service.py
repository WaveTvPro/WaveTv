...
if __name__ == '__main__':
    try:
        ADDON = xbmcaddon.Addon('plugin.video.WaveTv')
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(plugin.video.WaveTv)")
        
        if ADDON.getSetting('background.service') == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[plugin.video.WaveTv] Background service started...", xbmc.LOGDEBUG)
            Service()
            ...
            while not monitor.abortRequested():
                ...
                xbmc.log("[plugin.video.WaveTv] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                if monitor.waitForAbort(waitTime):
                    # Abort was requested while waiting. We should exit
                    break
                xbmc.log("[plugin.video.WaveTv] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                
    except source.SourceNotConfiguredException:
        pass  # ignore
    except Exception, ex:
        xbmc.log('[plugin.video.WaveTv] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)

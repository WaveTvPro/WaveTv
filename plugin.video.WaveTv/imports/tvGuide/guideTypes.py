...
ADDON = xbmcaddon.Addon(id='plugin.video.WaveTv')
...
class GuideTypes(object):
    ...
    filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.WaveTv', 'guides.ini'))
    ...
    def __init__(self):
        try:
            ...
            xbmc.log('[plugin.video.WaveTv] GuideTypes collected: %s' % str(self.guideTypes), xbmc.LOGDEBUG)
    ...
    def SectionMap(self, section):
        ...
        for option in options:
            try:
                ...
                if dict1[option] == -1:
                    xbmc.log('[plugin.video.WaveTv] skip: %s' % option, xbmc.LOGDEBUG)
            except:
    ...
    def getGuideDataItem(self, id, item):
        ...
        try:
            ...
            xbmc.log('[plugin.video.WaveTv] DataItem with index %s not found' % item, xbmc.LOGDEBUG)
        return value


    def getGuideById(self, id):
        xbmc.log('[plugin.video.WaveTv] Finding Guide with ID: %s' % id, xbmc.LOGDEBUG)
        ret = []
        for guide in self.guideTypes:
            if guide[self.GUIDE_ID] == int(id):
                ret = guide
                xbmc.log('[plugin.video.WaveTv] Found Guide with data: %s' % str(guide), xbmc.LOGDEBUG)
        return ret
...
if __name__ == '__main__':
    ...
    if ret >= 0:
        ...
        if xbmc.getCondVisibility('system.platform.android') and int(ver) < 15:
            ...
            xbmc.log('[plugin.video.WaveTv] Running on ANDROID with Kodi v%s --> using workaround!' % str(ver), xbmc.LOGDEBUG)
            filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.WaveTv', 'settings.xml'))

...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[plugin.video.WaveTv] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[plugin.video.WaveTv] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[plugin.video.WaveTv] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[plugin.video.WaveTv] Updating caches...', xbmc.LOGDEBUG)
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.WaveTv'))
    KEY = 'xmltv'
    INI_TYPE_Xhoans = 0
    INI_TYPE_CUSTOM = 1
    INI_FILE = 'addons.ini'
    ...
    def __init__(self, addon):
        ...
        if self.xmltvType == gType.CUSTOM_FILE_ID:
            ...
            if os.path.exists(customFile):
                ...
                xbmc.log('[plugin.video.WaveTv] Use local file: %s' % customFile, xbmc.LOGDEBUG)
                self.xmltvFile = customFile
            else:
                # Probably a remote file
                xbmc.log('[plugin.video.WaveTv] Use remote file: %s' % customFile, xbmc.LOGDEBUG)
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            try:
                ...
                xbmc.log('[plugin.video.WaveTv] strptime error with this date: %s' % dateString, xbmc.LOGDEBUG)

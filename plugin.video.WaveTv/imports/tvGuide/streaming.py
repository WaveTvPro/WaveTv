...
class StreamsService(object):
    def __init__(self, addon):
        if int(addon.getSetting('addons.ini.type')) == 0:
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.WaveTv', 'addons.ini'))
            xbmc.log('[plugin.video.WaveTv] Xhoans addons.ini is used', xbmc.LOGDEBUG)
        else:
            customFile = str(addon.getSetting('addons.ini.file'))
            if os.path.exists(customFile):
                path = customFile
            else:
                path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.WaveTv', customFile.split('/')[-1]))
            xbmc.log('[plugin.video.WaveTv] Custom addons.ini is used: %s' % path, xbmc.LOGDEBUG)

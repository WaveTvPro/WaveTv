...
class FileFetcher(object):
    ...
    basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.WaveTv'))
    filePath = ''
    fileUrl = ''
    addon = None
    ...
    def fetchFile(self):
        ...
        if fetch:
            ...
            if self.fileType == self.TYPE_REMOTE:
                xbmc.log('[plugin.video.WaveTv] file is in remote location: %s' % self.fileUrl, xbmc.LOGDEBUG)
                if not xbmcvfs.copy(self.fileUrl, tmpFile):
                    xbmc.log('[plugin.video.WaveTv] Remote file couldn\'t be copied: %s' % self.fileUrl, xbmc.LOGERROR)
            else:
                f = open(tmpFile, 'wb')
                xbmc.log('[plugin.video.WaveTv] file is on the internet: %s' % self.fileUrl, xbmc.LOGDEBUG)
            ...
            if os.path.getsize(tmpFile) > 256:
                ...
                xbmc.log('[plugin.video.WaveTv] file %s was downloaded' % self.filePath, xbmc.LOGDEBUG)
            else:
